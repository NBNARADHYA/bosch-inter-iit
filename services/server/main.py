...
async def pred_model_output(
    model: Optional[UploadFile] = File(None), model_name: Optional[str] = Form(None)
):
    ...
    if first_time:
        if model is None:
            raise HTTPException(
                status_code=400, detail="Model has to be uploaded or selected"
            )
...
async def get_most_confused_classes(
    model_name: str = Form(...), no_most: Optional[int] = Form(None)
):
    model_op_obj = Model_output(
    ...
    return {
        "most_confused_classes": model_op_obj.most_confused_classes(no_most=no_most)
    }
