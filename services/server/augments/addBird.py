...
class AddBird(DualTransform):
    ...
    def apply(self, img, **params):
        ...
        bird_mask = Image.open(self.mask_path + 'mask' + str(toss) + '.png').resize((self.mask_h,self.mask_w)).convert('RGB')
        bird_image = Image.open(self.mask_path + 'image' + str(toss) + '.png').resize((self.mask_h,self.mask_w)).convert('RGB')
