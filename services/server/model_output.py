...
class Model_output():
     def __init__(self, model_path = None, first_time = False, is_plot = False, is_run = False, is_cpu = True, data_path = 'test_dataset/test.csv', data_root = 'test_dataset/', batch_size=64):
      ...
      #      self.model_name = model['name']
        ...
        if first_time:
           ...
           self.metrics = {}
           self.pred()
           self.conf_matrix = confusion_matrix(self.labels, np.argmax(self.predictions, axis=1))
     ...
     def confusion(self):
        ...
        cm_display = ConfusionMatrixDisplay(self.conf_matrix, np.unique(np.array(self.labels))).plot(ax = ax)
     ...
     def wrost_acc_classes(self):
        cm = (self.conf_matrix.astype('float') / self.conf_matrix.sum(axis=1)[:, np.newaxis])*100
     ...
     def plot_precision_recall_curve(self,c):
        ...
        plt.legend()
        plt.show()
        path1 = "model_output/precision_recall_vs_confidence_" + self.model_path_name + "_class_" + str(c) + ".png"
        plt.savefig(path1)
        plt.clf()
        plt.close()
        ...
        plt.show()
        path2 = "model_output/precision_vs_recall_" + self.model_path_name + "_class_" + str(c) + ".png"
        plt.savefig(path2)
        plt.clf()
        plt.close()
     ...
     def plot_roc_curve(self, c):
        ...
        plt.show()
        path = "model_output/roc_curve_" + self.model_path_name + "_class_" + str(c) + ".png"
        plt.savefig(path)
        plt.clf()
        plt.close()
        return path
