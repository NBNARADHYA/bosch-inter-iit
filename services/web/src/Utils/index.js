...
export function generateTransformationRequestBody(
    transformation, history, params, img, originalDimensions, historyDimensions,
    preview) {
  const data = new FormData();
  if (transformation.parameters &&
      JSON.parse(transformation.parameters)
          .filter((para) => para.name === 'p')
          .length &&
      !params['p']) {
    params['p'] = 1.0;
  }
