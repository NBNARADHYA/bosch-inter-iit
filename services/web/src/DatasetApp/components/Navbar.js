...
export default function Navbar({
  ...
  let selectedUIOption = uiOptions.filter(({ links }) =>
    links.some((link) => currentPath === link)
  )[0];
  if (selectedUIOption) selectedUIOption = selectedUIOption.text;

  let selectedOption = menuOptions.filter(
    ...
    >
      <Toolbar>
        ...
        <Button
          ...
          aria-haspopup="true"
          className={navbarClasses.uiMenuOptions}
          id="step9"
        >
          <span className={navbarClasses.dropdownText}>
            ...
            <ExpandMoreIcon
              fontSize="small"
              className={navbarClasses.dropdownIcon}
            />
          </span>
        </Button>
        <Menu
        ...
        {showReset && (
          <Tooltip TransitionComponent={Zoom} title="Test another model">
            <IconButton
              color="inherit"
              className={navbarClasses.menuOptions}
              aria-label="Reset"
              component="span"
              onClick={() => {
                routerHistory.go(0);
        ...
        {selectedOption === "Apply transformations" && (
          <>
            <Tooltip
              ...
              title="Timeline of applied augmentations"
            >
              <IconButton
                color="inherit"
                className={navbarClasses.timelineButton}
                aria-label="Timeline"
                component="span"
