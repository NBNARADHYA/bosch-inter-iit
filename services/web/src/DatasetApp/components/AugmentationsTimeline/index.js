...
function AugmentationsTimeline(props) {
  ...
  const regenerateImage = useCallback((idx, newHistory) => {
    ...
    fetch(`${serverUrl}transform_image?preview=false`,
        ...
        .then(({img_path}) => {
          let modifiedHistory = newHistory.map((h, i) => {
            if (i === idx)
              return {...h, image : `${img_path}?${Date.now()}`};
            else
              return h;
          });
  ...
  <Drawer
  anchor = 'right'
  open = {isOpen} className = {classes.drawer} onClose = {closeDrawer} classes =
  {
    { paper: classes.drawerPaper, }
  } variant = "persistent" > <div className = {classes.root}>
                  <Grid container direction = "row" justify =
                       "center" alignItems = "center"><Grid item xs = {10}>
                  <Typography variant = "h5" component = "h1" align = "center">&
              nbsp;
      Applied Augmentations</Typography>    
 </Grid><Grid item xs = {2}>
  ...
  {history && !!history.length && <Timeline align="alternate">
    ...
    < TimelineStage
    ref = {lastRef} handleMouseOver = {() => setSelectedIndex(history.length -
                                                              1)} isSelected = {
        selectedIndex === history.length - 1} isLast
    handleRemove = {
      () => {
        const newHistory =
            history.filter((h, idx) => idx !== history.length - 1);
        setHistory(newHistory);
      }
    } image = {history[history.length - 1].image} name =
        {history[history.length - 1].name} parameters =
            {history[history.length - 1].parameters} even =
                {(history.length - 1) % 2 === 0} />            
    </Timeline >
    }
    </div>
</Drawer>)
